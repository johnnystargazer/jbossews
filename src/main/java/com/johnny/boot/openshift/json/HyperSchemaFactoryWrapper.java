...
public class HyperSchemaFactoryWrapper extends SchemaFactoryWrapper {
	...
	protected boolean ignoreDefaults = true;

	private static class HyperSchemaFactoryWrapperFactory extends
			WrapperFactory {
		@Override
	...
	public JsonObjectFormatVisitor expectObjectFormat(JavaType convertedType) {
		ObjectVisitor visitor = ((ObjectVisitor) super
				.expectObjectFormat(convertedType));
	...
	public JsonArrayFormatVisitor expectArrayFormat(JavaType convertedType) {
		ArrayVisitor visitor = ((ArrayVisitor) super
				.expectArrayFormat(convertedType));
	...
	protected void addHyperlinks(JsonSchema schema, JavaType type) {
		...
		if (rawClass.isAnnotationPresent(JsonHyperSchema.class)) {
			extraJsonSchema(schema, rawClass);
		} else {
			onNoJsonHyperSchema(schema, type);
	...
	private void extraJsonSchema(JsonSchema schema, Class<?> rawClass) {
		JsonHyperSchema hyperSchema = rawClass
				.getAnnotation(JsonHyperSchema.class);
		...
		for (int i = 0; i < links.length; i++) {
			...
			linkDescriptionObjects[i] = new LinkDescriptionObject()
					...
					.setRel(link.rel())
					.setMethod(
							ignoreDefaults && "GET".equals(link.method()) ? null
									: link.method())
					.setEnctype(
							ignoreDefaults
									&& "application/json".equals(link.enctype()) ? null
									: link.enctype())
					.setTargetSchema(fetchSchema(link.targetSchema()))
					.setSchema(fetchSchema(link.schema()))
					.setMediaType(
							ignoreDefaults
									&& "application/json".equals(link
											.mediaType()) ? null : link
									.mediaType()).setTitle(link.title());
		}
	...
	protected JsonSchema fetchSchema(Class<?> targetSchema) {
		if (provider instanceof DefaultSerializerProvider
				&& targetSchema != void.class) {
			JavaType targetType = provider.constructType(targetSchema);
			try {
				if (visitorContext != null) {
					String seenSchemaUri = visitorContext
							.getSeenSchemaUri(targetType);
				...
				((DefaultSerializerProvider) provider).acceptJsonFormatVisitor(
						targetType, targetVisitor);
